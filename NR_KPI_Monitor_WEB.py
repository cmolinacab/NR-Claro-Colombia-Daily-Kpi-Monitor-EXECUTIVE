...
kpis=['NR_5020d','NR_5080a','NR_5124b','NR_5150a']
...
#project_url = project_name.replace('-api', '')
app = dash.Dash(__name__, external_stylesheets=external_stylesheets, title='5G Cluster Monitor')#,url_base_pathname=f'/{project_url}/nr-kpi-monitor/')
...
app.layout =html.Div([
    ...
    html.Div([
        ...
        dcc.Graph(id='NR_5150a', style={"height": 270}),
    ...
    dash_table.DataTable(
            id='table_black_list',
            columns=[{"name": i, "id": i} for i in ['Date','Site','NR_5150a']],
    ...
    dash_table.DataTable(
            id='table_no_traffic',
            columns=[{"name": i, "id": i} for i in ['Date','cluster','Site','NR_5150a']], 
            style_cell={'textAlign': 'center', 'fontSize':12},
            style_header={
...
@app.callback(
    ...
    Input('NR_5150a', 'clickData'))
def display_click_data(d_NR_5020d,d_NR_5080a,d_NR_5124b,d_NR_5150a):
    ...
    elif 'NR_5150a' in changed_id:
        cluster=d_NR_5150a['points'][0]['label']
        kpi_graph='NR_5150a'
...
#Actualizar lista de procesos
    ...
    avail_off=pd.DataFrame(columns=['Date','cluster','Site','NR_5150a'])
    ...
    try:
        ...
        print('Survival Seconds:',survival_time.seconds)


        if survival_time.seconds>350:
            ...
            for i, j in tasks.iterrows():
                ...
                KPI_Site_Cluster = pd.DataFrame.from_dict(data).dropna(subset=['NR_5150a'])
                ...
                data = json.loads(x.text)
                KPI_Site_Day=pd.DataFrame.from_dict(data).dropna(subset=['NR_5150a'])
            ...
            KPI_all.to_csv('KPI_all.csv', index=False)
            ...
            print('No se refresca, se leen los temporales')

        KPI_Site_Day_All=pd.read_csv('NR_Monitor_KPI_Site_Day.csv')
        KPI_all_ini=pd.read_csv('KPI_all.csv')
        ...
        kpis_th=[('NR_5020d',65,'5G NSA Non Stand Alone call accessibility, 5G side'),('NR_5080a',0,'5G NSA PDCP SDU data volume transmitted without repetitions in DL'),
                ('NR_5124b',0,'5G NSA Average number of NSA users in selected area'),('NR_5150a',99,'5G Cell availability ratio excluding planned unavailability periods')]
        ...
        if survival_time.seconds>350:
            ...
            KPI_site_offender_hour.to_csv('KPI_site_offender_hour.csv', index=False)



        KPI_site_offender_hour=pd.read_csv('KPI_site_offender_hour.csv')
        KPI_site_offender_hour['NR_5150a']=KPI_site_offender_hour['NR_5150a'].apply(lambda x: round(x, 4) if x!=None else None )


        avail_off=KPI_site_offender_hour[KPI_site_offender_hour['NR_5150a']<99]
        ...
        if survival_time.seconds>350:
            ...
            avail_black.to_csv('avail_black.csv', index=False)



        avail_black=pd.read_csv('avail_black.csv')


        avail_black['NR_5150a']=avail_black['NR_5150a'].apply(lambda x: round(x, 2) if x!=None else None )

        black_info=pd.DataFrame(columns=['Date','NR_5150a','Site'])
...
def execute_functions(n):
    ...
    data_hist=data_hist[data_hist['NR_5150a']<99]
